...
def rescale(img, p):
    return cv2.resize(img, (int(img.shape[1]*p), int(img.shape[0]*p)))

class CARLAMultiDataset(BaseDataset):
    """
    Input params:
    ...
    def __getitem__(self, index):
        ...
        if (self._stage == 'train'):
            ...
            sim_image = [rescale(rescale(sim_image, 1/pow(2, i)), pow(2, i)) for i in range(4)]
            real_image = [rescale(rescale(real_image, 1/pow(2, i)), pow(2, i)) for i in range(4)]

        #Image formatting
        sim_image = [np.dstack([s] * 1) for s in sim_image]
        real_image = [np.dstack([r] * 1) for r in real_image]

        #Some image augmentation
        # image = seg(image=image)

        #Apply defined transforms to image from constructor (will convert to tensor)
        sim_image = [self._transform(s) for s in sim_image]
        real_image = [self._transform(r) for r in real_image]
